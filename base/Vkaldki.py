...
# Переменная для хранения выбранного партнера
selected_partner = None  # Инициализация переменной
...
def get_partners():
    ...
    try:
        with pyodbc.connect(connection_string) as conn:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT p.[Наименование партнера], p.[Тип партнера], p.[Директор], p.[Телефон партнера], p.[Рейтинг], SUM(pp.[Количество продукции]) AS TotalSales
                FROM Партнеры p
